...
- **Collector Run Contributors endpoint.** The synchronous PowerQuery call MUST target the SDL/xdr host at `{{SDL_XDR_URL}}/api/powerQuery` (e.g. `https://xdr.us1.sentinelone.net/api/powerQuery`) with `use_authentication_data:true` bound to the **"SentinelOne SDL"** (Bearer) connection. Do NOT use `{{Connection.url}}/sdl/api/powerQuery`, that path is wrong and the action fails (tenant-confirmed 2026-07-06). Bind the SDL connection on BOTH HTTP actions (Run Contributors and Publish); binding only Publish leaves Run Contributors unauthenticated.
