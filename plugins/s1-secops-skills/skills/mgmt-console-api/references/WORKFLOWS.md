...
- **Reaching for an HA watchdog when a scheduled rule would do**: "run this query on a schedule and alert if rows > 0" belongs in `cloud-detection/rules` with `queryType: "scheduled"` + `queryLang: "2.0"`. The HA watchdog is the right answer when the scheduled evaluator rejects the query (`datasource`, `savelookup`, `now()`, time-shifted `timebucket`, unrestricted joins, results past the ~1,000-row cap, or an alert body built from the result). Both are supported; pick by what the query needs.
