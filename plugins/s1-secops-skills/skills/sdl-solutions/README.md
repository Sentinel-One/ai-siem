...
| Custom detection exclusions | Suppress known-good noise in a STAR Custom Detection rule over a third-party or EDR source, for all three rule types: single-event and correlation rules carry the exclusion as an inline `AND NOT (...)` negative list; scheduled rules key against a CSV exclusion list (assets by IP/CIDR/host, or custom domains/users/values) via a lookup anti-join, with an exclusion-effectiveness dashboard | [guide](../../docs/solutions/custom-detection-exclusions.md) | [`references/custom-detection-exclusions.md`](references/custom-detection-exclusions.md) |
...
| Alert noise reduction | Find the (source, signature) pairs flooding the alert queue, separate ingested and already-actioned noise (block/drop/sinkhole/reset) from real S1 detections, recommend an ingestion-severity filter, auto-resolve already-mitigated alerts with a note, optionally preserve signal-worthy categories as one correlation rule, and ship a noise-vs-signal dashboard. Product/source/signature/action values are discovered live; nothing is hardcoded | [guide](../../docs/solutions/alert-noise-reduction.md) | [`references/alert-noise-reduction.md`](references/alert-noise-reduction.md) |
...
| Tune out known-good noise without losing real signal | Custom detection exclusions let an analyst suppress an allowlisted set of assets or values from any STAR rule: inline in the rule body for single-event and correlation rules, or, for scheduled rules, with one CSV applied at detection time via a lookup anti-join plus a dashboard that shows exactly what each list is suppressing so an over-broad exclusion that hides a real threat is caught. |
...
| Quiet an alert queue drowning in third-party noise | Alert noise reduction measures how much of the queue one ingested source accounts for, proves the source already blocked or sinkholed each event, and deploys the fixes: an ingestion-severity filter recommendation with the exact volumes attached, an auto-resolve flow that closes already-mitigated alerts with a note, and a noise-vs-signal dashboard, so analysts triage real detections instead of already-handled logs. |
...
- *"My alert queue is flooded, reduce the noise and auto-close the already-blocked firewall alerts"*
