...
The image is published to `sentinelone/secops-skills`. Tags follow semver plus `:latest` and `:sha-<short>`. The matching CI workflow is at [`.github/workflows/docker-publish.yml`](../.github/workflows/docker-publish.yml).
...
|---|---|---|
| Image version (`IMAGE_VERSION`) | this repo | `1.4.5` |
...
| `purple-mcp` | git, `Sentinel-One/purple-mcp` | `07d4992` (`v0.7.0`, 2026-06-26) |
...
`IMAGE_VERSION` is the version tag the image is published under, alongside the moving `latest`. It is its own counter, independent of the MCP versions inside: bump it whenever anything that changes the image bytes changes, which is the Dockerfile, the dispatcher, the bundled `CLAUDE.md`, the `s1-secops-mcp` source now that it is `COPY`'d, or any pin above.
...
docker run -i --rm s1-mcps:latest versions
...
docker run --rm --entrypoint sh s1-mcps:latest -c 'command -v npm || echo absent'
```
