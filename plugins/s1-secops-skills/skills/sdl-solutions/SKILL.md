...
description: Deploy packaged, repeatable SentinelOne Singularity Data Lake (SDL) solutions into a site from one prompt. Use when the user wants to onboard, deploy, or roll out a whole SDL solution. Catalog: (1) data source onboarding (raw to OCSF, enrichment, dashboard, MITRE detections, threat response); (2) asset enrichment from the Asset Inventory; (3) UEBA behavioural anomaly detection (per action/principal z-score: SPIKE/DROP/SILENT/NEW); (4) per-device ingest health monitoring (7-day baseline: spike/drop/lag/silence/parser drift); (5) detection exclusions, suppress known-good noise in a STAR Custom Detection rule over a third-party or EDR source, as either a STAR single-event rule (inline hardcoded 'AND NOT (field in:anycase (...))' exclusion list in a boolean S1QL body) or a scheduled PowerQuery detection (CSV exclusion list of assets by IP/CIDR/host or custom domains/users/values, applied via a lookup anti-join) with an effectiveness dashboard; the solution asks which rule type first. (6) Risk-Based Alerting (RBA): publish noisy-but-interesting observations as low-noise risk events into a 'risk' index instead of alerting individually, accumulate risk per user/host object amplified by asset-derived risk factors, and fire one high-fidelity alert when a 24h cumulative-score or 7d multi-MITRE-tactic threshold is crossed; deploys contributors, a risk-factor table, a scheduled collector flow, four incident rules, and a dashboard. Triggers: 'onboard a new source', 'deploy UEBA/asset enrichment/ingest health', 'add a detection exclusion for a source', 'exclude these assets/domains/users from a detection', 'exclude my team from a detection', 'deploy risk-based alerting', 'set up RBA', 'risk scoring for users/hosts'. NOT for one-off queries or standalone parser authoring.
---
