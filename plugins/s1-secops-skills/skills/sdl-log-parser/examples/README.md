...
| `09-aws-cloudtrail-assumerole-ocsf.json` | AWS CloudTrail `sts:AssumeRole` records → OCSF **Authentication (3002)**. Shows the strictDottedJson-capture idiom (strict* variants keep each array as one JSON value; non-strict gron/dottedJson expand arrays into `[N]`-indexed attributes, see `references/parse-directives.md`), the full `user.*` / `session.*` / `api.*` / `cloud.*` / `tls.*` fan-out, and how to surface array payloads (`resources[]`) as a queryable JSON blob + a scalar `resource_uid`. |
