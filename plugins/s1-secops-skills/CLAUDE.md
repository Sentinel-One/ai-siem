...
- **Indexed** array fields (`resources[0].*`, `finding_info.attacks[0].*`) are rejected EVERYWHERE, HTTP 400, in `columns` and `group by` as well as in a filter predicate. Measured, correcting an earlier note that claimed `columns` and `group by` worked. Quoting the name does not help: the parser treats `"finding_info.attacks[0].technique.uid"` as a string literal and hands the literal back as the value, so the query looks like it succeeded and returned one row of column names. That silent form is the dangerous one. Use the **wildcard** accessor instead, `resources[*].name` / `finding_info.attacks[*].tactic.uid`, which does work as a group and filter key (values come back JSON-array-wrapped), or group on the flat parent field and parse the JSON client-side. Filter on `class_uid` or `finding_info.title`, or take asset context from `get_alert` plus the inventory record.
